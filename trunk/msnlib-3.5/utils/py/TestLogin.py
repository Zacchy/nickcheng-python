...
#            debug(str(fd.fileno()) + ' <<< ' + buf)
    ...
    def login(self):
        ...
        # version information
        self._send('VER', 'MSNP15 MSNP14 MSNP13 CVR0')
        ...
        self._send('CVR', '0x0409 winnt 6.0 i386 MSNMSGR 8.1.0178 msmsgs ' + self.email)
        self._recv()	# we just don't care what we get

        # ask for notification server
        self._send('USR', 'SSO I ' + self.email)
        ...
        # get GCF command, and get the length of the following XML data
        r = self._recv()
        xmlLength = int(r[2])
        	
        r = self._recv()
        xmlContent = ' '.join(r)
        r = xmlContent[xmlLength:].split(' ', 2)
        print r
        	
        if r[0] != 'USR':
            raise 'AuthError', r
        #hash = string.split(r[2])[2]
        refURI = string.split(r[2])[2]
        nonce = string.split(r[2])[3]

        # get and use the passport id
        #passportid = self.passport_auth(hash)
        	
        # get and use security-token and proof-token
        securityToken, proofToken, tokenContacts = self.msnp15_auth(refURI)
        blob = self.mbi_encrypt(proofToken, nonce)
        self._send('USR', 'SSO S %s %s' % (securityToken, blob))
        ...
        r = self._recv() # SBS 0 null
        r = self._recv() # MSG Hotmail Hotmail <length>
        l = int(string.split(r[2])[1]) # get the length of MSG command
        r = self._recvmsg(l)
        
        self.testABService(tokenContacts)

        return 1

    def testABService(self, tC):
        ''''''
        import httplib
                debug('testABService begin')
        pattern = '<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/"><soap:Header><ABApplicationHeader xmlns="http://www.msn.com/webservices/AddressBook"><ApplicationId>09607671-1C32-421F-A6A6-CBFAA51AB5F4</ApplicationId><IsMigration>false</IsMigration><PartnerScenario>Initial</PartnerScenario></ABApplicationHeader><ABAuthHeader xmlns="http://www.msn.com/webservices/AddressBook"><ManagedGroupRequest>false</ManagedGroupRequest><TicketToken>%s</TicketToken></ABAuthHeader></soap:Header><soap:Body><ABFindAll xmlns="http://www.msn.com/webservices/AddressBook"><abId>00000000-0000-0000-0000-000000000000</abId><abView>Full</abView><deltasOnly>false</deltasOnly><lastChange>0001-01-01T00:00:00.0000000-08:00</lastChange></ABFindAll></soap:Body></soap:Envelope>'
        content = pattern % (tC)
        
        h	= httplib.HTTPS('contacts.msn.com')        h.putrequest('POST', '/abservice/abservice.asmx')        h.putheader('SOAPAction','http://www.msn.com/webservices/AddressBook/ABFindAll')        h.putheader('Content-Type', 'text/xml; charset=utf-8')        h.putheader('Host', 'contacts.msn.com')        h.putheader('Content-Length', str(len(content)))        h.endheaders()        	        debug('SSL Connecting...')        h.send(content)        	        debug('SSL POST & Get the reply')        errcode, errmsg, headers = h.getreply()        reply	= h.file.read()#        soup = BeautifulStoneSoup(reply)
...
#        debug('email: ' + self.email)#        debug('pwd: ' + self.pwd)#        debug('refURI: ' + refURI)        	
        ...
        reply	= h.file.read()
        soup = BeautifulStoneSoup(reply)        	
        if errcode != 200 or soup.find('s:fault') != None:
            raise 'AuthError', [911, 'SSL Auth failed']
        print reply
        	
        sToken = soup.findAll('wst:requestsecuritytokenresponse')[1].find('wsse:binarysecuritytoken').string
        pToken = soup.findAll('wst:requestsecuritytokenresponse')[1].find('wst:binarysecret').string
        tokenContacts = soup.findAll('wst:requestsecuritytokenresponse')[2].find('wsse:binarysecuritytoken').string
#        sToken = re.findall(r'"Compact1">([^<]*)', reply)[0]
#        pToken = re.findall(r'wst:BinarySecret>([^<]*)', reply)[-2]
        	
        return (sToken, pToken, tokenContacts)
        	
    def mbi_encrypt(self, pToken, nonce):
        '''abcd
        efgh'''
        import struct
    ...
    def passport_auth(self, hash):
        ...
        nexus = urllib.urlopen('https://loginnet.passport.com/RST.srf')
        h = nexus.headers
        purl = h['PassportURLs']
    ...
    def read(self, nd = None):
        ...
        # Added by nickcheng
        elif type == 'UUX': self.cb.uux(self, type, tid, params)
        elif type == 'UBX': self.cb.ubx(self, type, tid, params)
        elif type == 'SBS': self.cb.sbs(self, type, tid, params)
